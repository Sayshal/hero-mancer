...
export class DOMManager {
  ...
  static async updateBackgroundSummary() {
    ...
    try {
      ...
      summary.innerHTML = await getTextEditor().enrichHTML(content);
    } catch (error) {
  ...
  static async updateClassRaceSummary() {
    ...
    try {
      ...
      summary.innerHTML = await getTextEditor().enrichHTML(fallbackContent);
    }
  }
  ...
  static async #updateSummaryHTML(selectedAbilities) {
    ...
    if (selectedAbilities.length >= 2) {
      ...
      abilitiesSummary.innerHTML = await getTextEditor().enrichHTML(content);
    } else {
  ...
  static async #updateBiographyReview(container) {
    ...
    if (bioData.backstory) {
      ...
      <div class="backstory-text">${await getTextEditor().enrichHTML(bioData.backstory)}</div>
    `;
        ...
        if (backgroundItems[0].isStartingWealth) {
          ...
          const itemsHtml = await Promise.all(
            backgroundItems.map(async (item) => {
              return `<div class="equipment-item">${await getTextEditor().enrichHTML(`@UUID[${item.uuid}]{${item.name}}`)}</div>`;
            })
          );
